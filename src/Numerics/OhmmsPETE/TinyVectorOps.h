...
template <class T1> struct BinaryReturn<T1, std::complex<T1>, OpMultiply> {
  typedef std::complex<T1> Type_t;
};

template <class T1> struct BinaryReturn<std::complex<T1>, T1, OpMultiply> {
  typedef std::complex<T1> Type_t;
};
...
struct OTAssign<TinyVector<T1, D>, TinyVector<T2, D>, OP> {
  inline static void apply(TinyVector<T1, D> &lhs, const TinyVector<T2, D> &rhs,
                           OP op) {
...
struct OTBinary<TinyVector<T1, D>, T2, OP> {
  typedef typename BinaryReturn<T1, T2, OP>::Type_t Type_t;
  inline static TinyVector<Type_t, D> apply(const TinyVector<T1, D> &lhs,
                                            const T2 &rhs, OP op) {
    TinyVector<Type_t, D> ret;
    for (unsigned d = 0; d < D; ++d)
      ret[d] = op(lhs[d], rhs);
...
struct OTDot<TinyVector<T1, D>, TinyVector<T2, D>> {
  ...
  inline static Type_t apply(const TinyVector<T1, D> &lhs,
...
struct OTDot<TinyVector<T1, 3>, TinyVector<std::complex<T1>, 3>> {
  typedef T1 Type_t;
  inline static Type_t apply(const TinyVector<T1, 3> &lhs,
                             const TinyVector<std::complex<T1>, 3> &rhs) {
    return lhs[0] * rhs[0].real() + lhs[1] * rhs[1].real() +
           lhs[2] * rhs[2].real();
  }
};
...
struct OTDot<TinyVector<std::complex<T1>, 3>, TinyVector<T2, 3>> {
  typedef T1 Type_t;
  inline static Type_t apply(const TinyVector<std::complex<T1>, 3> &lhs,
                             const TinyVector<T2, 3> &rhs) {
    return lhs[0].real() * rhs[0] + lhs[1].real() * rhs[1] +
           lhs[2].real() * rhs[2];
  }
};
...
struct OTDot<TinyVector<std::complex<T1>, 3>, TinyVector<std::complex<T2>, 3>> {
  typedef typename BinaryReturn<std::complex<T1>, std::complex<T2>,
                                OpMultiply>::Type_t Type_t;
  inline static Type_t apply(const TinyVector<std::complex<T1>, 3> &lhs,
                             const TinyVector<std::complex<T2>, 3> &rhs) {
    return std::complex<T1>(
        lhs[0].real() * rhs[0].real() - lhs[0].imag() * rhs[0].imag() +
...
struct OTCross<TinyVector<T1, D>, TinyVector<T2, D>> {
  typedef typename BinaryReturn<T1, T2, OpMultiply>::Type_t Type_t;
  inline static TinyVector<Type_t, D> apply(const TinyVector<T1, D> &a,
