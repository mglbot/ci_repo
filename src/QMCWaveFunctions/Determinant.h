...
template <class T>
inline int getGetriWorkspace(T *restrict x, int n, int lda,
                             int *restrict pivot) {
  T work;
  int lwork = -1;
...
#pragma omp master
  std::cout << tag << " difference between matrices (average per element) = "
            << error / nrows / nrows << std::endl;
}
...
struct DiracDeterminant : public WaveFunctionComponentBase {
  ...
  inline ValueType ratio(ParticleSet &P, int iel) {
    ...
    curRatio =
        inner_product_n(psiV.data(), psiMinv[iel - FirstIndex], nels, czero);
    return curRatio;
  }
  ...
  inline void acceptMove(ParticleSet &P, int iel) {
    const int nels = psiV.size();
    updateRow(psiMinv.data(), psiV.data(), nels, nels, iel - FirstIndex,
              curRatio);
