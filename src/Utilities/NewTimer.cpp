...
void TimerManagerClass::collate_stack_profile(StackProfileData &p) {
  ...
  for (int i = 0; i < TimerList.size(); ++i) {
    ...
    for (; stack_id_it != timer.get_per_stack_total_time().end();
         stack_id_it++) {
      ProfileData pd;
...
XMLNode *TimerManagerClass::output_timing(XMLDocument &doc) {
  ...
  timing_root->InsertEndChild(
      MakeTextElement(doc, "max_stack_level_exceeded",
                      timer_max_level_exceeded ? "yes" : "no"));
  timing_root->InsertEndChild(MakeTextElement(
      doc, "max_timers_exceeded", max_timers_exceeded ? "yes" : "no"));
  ...
  for (int i = 0; i < p.names.size(); i++) {
    ...
    timer->InsertEndChild(
        MakeTextElement(doc, "time_incl", std::to_string(p.timeList[i])));
    timer->InsertEndChild(
        MakeTextElement(doc, "time_excl", std::to_string(p.timeExclList[i])));
    timer->InsertEndChild(
        MakeTextElement(doc, "calls", std::to_string(p.callList[i])));
